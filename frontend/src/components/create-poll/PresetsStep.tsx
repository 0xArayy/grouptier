...
export type SelectResult = { kind: 'template'; preset: Preset } | { kind: 'poll'; id: string };
...
type ListItem = { kind: 'template'; data: PublicTemplate } | { kind: 'poll'; data: PublicPoll };
...
export function PresetsStep({ busy, error, onBack, onSelect }: Props) {
  ...
  const filtered = useMemo((): ListItem[] => {
    ...
    const userPolls =
      category === 'hot' || category === 'official'
        ? []
        : polls.filter((p) => {
          ...
          <div className={styles.emptyState}>
            ...
            <button
              ...
              onClick={() =>
                onSelect({
                  kind: 'template',
                  preset: { emoji: '🎯', name: query || 'Свой шаблон', options: [] },
                })
              }
            >
        ...
        {!loading && filtered.length > 0 && (
          <div className={styles.list}>
                ...
                return (
                  ...
                  >
                    ...
                    <div className={styles.cardBody}>
                      ...
                      <div className={styles.cardMeta}>GroupTier · {formatUses(t.uses_7d)} использовали</div>
            ...
            <button
              ...
              className={styles.dashedBtn}
              onClick={() =>
                onSelect({ kind: 'template', preset: { emoji: '🎯', name: 'Свой шаблон', options: [] } })
              }
              disabled={busy}
            >
              <span>+</span> Создать пустой шаблон
