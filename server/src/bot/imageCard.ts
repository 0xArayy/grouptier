...
function ensureFonts() {
  ...
  for (const [pkg, name] of list) {
    try {
      GlobalFonts.registerFromPath(_require.resolve(pkg), name);
    } catch (err) {
      console.warn(`imageCard: font "${name}" failed:`, err);
    }
  }
}
 ...
 */
function layoutTitle(ctx: Ctx, text: string, maxW: number, startPx: number, font: string): TitleLayout {
  ctx.letterSpacing = '0px';
...
export async function generateVotingCardAsync(name: string, optionCount: number): Promise<Buffer> {
  ...
  return getPool().run(
    { task: 'voting', args: [name, optionCount] },
